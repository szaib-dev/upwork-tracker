...
function mapSharedRow(row: Record<string, unknown>): Proposal {
  return {
    ...
    socials: typeof row.socials === "object" && row.socials ? (row.socials as Record<string, string>) : {},
...
export default function SharedAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: accessRows, error: accessError } = await supabase.rpc("get_analytics_share_access", { p_token: token });
      ...
      const { data: rows, error: rowsError } = await supabase.rpc("get_shared_proposals", { p_token: token });
      if (rowsError) {
      ...
      setProposals(((rows as Record<string, unknown>[] | null) ?? []).map(mapSharedRow));
      setLoading(false);
    };
  ...
  const monthFiltered = useMemo(() => {
    return proposals.filter((p) => monthFilter === "all" || p.dateSent.startsWith(monthFilter));
  }, [proposals, monthFilter]);

  const filtered = useMemo(() => {
    return monthFiltered.filter((p) => statusFilter === "All" || p.status === statusFilter);
    ...
    return (
      <div style={fullPage}>
        <div style={panel}>
          ...
          <Link href="/" style={linkBtn}>Go Home</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={fullPage}>
        <div style={panel}>
          ...
          <div style={{ color: "var(--muted)", marginBottom: 10 }}>This shared analytics link does not exist.</div>
          <Link href="/" style={linkBtn}>Go Home</Link>
        </div>
      </div>
    );
      ...
      <div style={fullPage}>
        <div style={{ ...panel, maxWidth: 520 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <Link href="/" style={linkBtn}>Email Sign In</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh", color: "var(--text)" }}>
      <div style={{ borderBottom: "1px solid var(--border)", padding: "10px 12px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 800 }}>Shared Analytics</div>
          <div style={{ fontSize: 12, color: "var(--muted)" }}>{access.title || "Team Analytics"}</div>
        </div>
        <Link href="/" style={linkBtn}>Open App</Link>
      </div>

      <div style={{ padding: "12px 14px" }}>
        <div style={{ background: "var(--bg-soft)", border: "1px solid var(--border)", borderRadius: 12, padding: 12, marginBottom: 12, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <button onClick={() => setViewMode("dashboard")} style={toggleBtn(viewMode === "dashboard")}>Dashboard</button>
          <button onClick={() => setViewMode("analytics")} style={toggleBtn(viewMode === "analytics")}>Analytics</button>
        </div>

        <div style={{ background: "var(--bg-soft)", border: "1px solid var(--border)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <label style={{ fontSize: 11, color: "var(--muted)", marginRight: 8 }}>Month:</label>
        ...
        {viewMode === "dashboard" ? (
          <>
            ...
            <Filters currentFilter={statusFilter} setFilter={setStatusFilter} proposals={monthFiltered} />
