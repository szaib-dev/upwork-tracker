...
export default function ProposalTracker() {
      ...
      <div className="w-full max-w-[1180px] grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
        ...
        <div className="hidden md:block">
          ...
          <p className="mt-7 text-base text-neutral-500 font-light leading-relaxed max-w-sm">
            A fast, quiet workspace for proposals, follow-ups, analytics, and client tracking — built for freelancers.
          </p>
          ...
          <div className="mt-12 flex flex-col gap-3">
            {[
              ...
              <div key={f.label} className="flex items-center gap-3">
                ...
                <span style={{ fontSize: 13, color: "#444", fontWeight: 300 }}>{f.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 24 }}>
            <h2 style={{ fontSize: 20, fontWeight: 300, color: "#fff", margin: 0, letterSpacing: "-0.01em" }}>
              {isRecoveryMode ? "Reset password" : isForgotMode ? "Forgot password" : isSignUp ? "Create an account" : "Welcome back"}
            </h2>
            <p style={{ fontSize: 13, color: "#444", marginTop: 5, fontWeight: 300 }}>
              {isRecoveryMode ? "Enter a new password below." : isForgotMode ? "We'll send a reset link to your email." : isSignUp ? "Get started for free." : "Sign in to your workspace."}
            </p>
          </div>
          ...
          <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {authError && (
              <div style={{ fontSize: 13, color: "#f87171", fontWeight: 300, padding: "8px 12px", background: "rgba(248,113,113,0.07)", border: "1px solid rgba(248,113,113,0.15)", borderRadius: 8 }}>
                {authError}
              </div>
            )}
            {authNotice && (
              <div style={{ fontSize: 13, color: "#a3a3a3", fontWeight: 300, padding: "8px 12px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8 }}>
                {authNotice}
              </div>
            )}

            <button type="submit" disabled={authLoading} className="auth-submit" style={{ marginTop: 6 }}>
              {authLoading
                ? "Please wait..."
                : isRecoveryMode
                ? "Update Password"
            ...
            {!isSignUp && !isForgotMode && !isRecoveryMode && (
              <button
                type="button"
                onClick={() => { setIsForgotMode(true); setAuthError(null); setAuthNotice(null); }}
  ...
  return (
    <div className="" style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      <AppHeader onAddProposal={() => setDrawerOpen(true)} />

      <main style={{ width: "100%", padding: "10px 10px 20px", minWidth: 0, maxWidth: 1420, margin: "0 auto" }} className="home-main">
