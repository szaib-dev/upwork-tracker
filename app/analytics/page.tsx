...
export default function AnalyticsDashboard() {
  ...
  const advanced = useMemo(() => {
    ...
    const leadRate = filtered.length ? Math.round((leads / filtered.length) * 100) : 0;
    const boostedRate = filtered.length ? Math.round((boosted / filtered.length) * 100) : 0;
        ...
        {loading ? (
          ...
          <>
            <section style={{ ...card, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8 }}>
              ...
              <MiniMetric label="Boosted Rate" value={`${advanced.boostedRate}%`} />
            </section>
