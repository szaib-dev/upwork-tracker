...
function InsightCard({ title, value, note, color }: { title: string; value: string; note: string; color: string }) {
  return (
...
export default function Insights({ proposals, monthOrder = "desc" }: { proposals: Proposal[]; monthOrder?: "asc" | "desc" }) {
  const data = useMemo(() => {
    const monthMap: Record<string, { sent: number; hired: number; connects: number; revenue: number }> = {};
    const countryMap: Record<string, { total: number; hired: number }> = {};

    proposals.forEach((p) => {
      const monthKey = (p.dateSent || "").slice(0, 7);
      if (monthKey) {
        if (!monthMap[monthKey]) monthMap[monthKey] = { sent: 0, hired: 0, connects: 0, revenue: 0 };
      ...
      if (p.clientCountry) {
        if (!countryMap[p.clientCountry]) countryMap[p.clientCountry] = { total: 0, hired: 0 };
    ...
    const bestMonth = monthsAsc.reduce<(typeof monthsAsc)[number] | null>((best, month) => {
      if (!best) return month;
      if (month.hired > best.hired) return month;
      if (month.hired === best.hired && month.winRate > best.winRate) return month;
      return best;
    }, null);
    ...
    const topCountry = Object.entries(countryMap)
      ...
      .map(([country, value]) => ({ country, ...value, winRate: value.total ? Math.round((value.hired / value.total) * 100) : 0 }))
      .at(0);
    ...
    const connectEfficiency = [
      ...
      .map((bucket) => {
        const set = proposals.filter((p) => p.connects >= bucket.min && p.connects <= bucket.max);
  ...
  const monthDelta = data.thisMonth && data.prevMonth ? data.thisMonth.sent - data.prevMonth.sent : 0;

  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ fontSize: 11, color: "var(--muted)", letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 700, marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(4, minmax(180px, 1fr))", marginBottom: 14 }}>
        ...
        <InsightCard title="Best Connect Band" value={data.bestConnectBand?.label ?? "-"} note={data.bestConnectBand ? `${data.bestConnectBand.winRate}% win rate` : "Not enough data"} color="#b06eff" />
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
            <thead>
              <tr>
                {["Month", "Sent", "Hired", "Win Rate", "Connects", "Revenue"].map((head) => (
                  <th key={head} style={{ textAlign: "left", padding: "8px 10px", color: "var(--muted)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border)" }}>
                    {head}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {data.months.map((month) => (
                <tr key={month.month}>
                  <td style={{ padding: "8px 10px", color: "var(--text)", fontFamily: "monospace" }}>{month.label}</td>
                  <td style={{ padding: "8px 10px", color: "var(--muted)" }}>{month.sent}</td>
                  <td style={{ padding: "8px 10px", color: "var(--success)" }}>{month.hired}</td>
                  <td style={{ padding: "8px 10px", color: "var(--primary)" }}>{month.winRate}%</td>
                  <td style={{ padding: "8px 10px", color: "#ffd060" }}>{month.connects}</td>
                  <td style={{ padding: "8px 10px", color: "#b06eff" }}>${month.revenue.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
