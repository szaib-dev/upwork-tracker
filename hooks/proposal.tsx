...
function normalizeRow(
	...
	return {
		...
		createdAt:
			(get("createdAt", "created_at") as string | undefined) ?? undefined,
	};
}
...
function normalizeUpdate(field: keyof Proposal, value: unknown): unknown {
	if (
		(field === "followUpAt" || field === "replyDate") &&
		typeof value === "string"
	) {
			...
			.on(
				...
				(payload) => {
					...
					if (payload.eventType === "DELETE") {
						setProposals((curr) =>
							curr.filter((item) => item.id !== normalized.id),
						);
						return;
					}
					...
					if (payload.eventType === "INSERT") {
						setProposals((curr) =>
							curr.some((item) => item.id === normalized.id)
								? curr
								: [normalized, ...curr],
						);
						return;
	...
	const updateProposal = useCallback(
		async (id: number, field: keyof Proposal, value: unknown) => {
			...
			setProposals((curr) =>
				curr.map((item) => {
					...
					if (field === "viewed") {
						...
						if (viewedNext && item.status === "Sent")
							localPatch.status = "Viewed";
						if (!viewedNext && item.status === "Viewed")
							localPatch.status = "Sent";
			...
			for (const [patchKey, patchValue] of Object.entries(localPatch)) {
				patch[toDbColumn(patchKey as keyof Proposal, namingStyleRef.current)] =
					patchValue;
			}

			let query = supabase.from("proposals").update(patch).eq("id", id);
			if (supportsUserScopeRef.current && userId)
