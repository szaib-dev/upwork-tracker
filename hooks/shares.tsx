...
type UseAnalyticsSharesResult = {
  ...
  loading: boolean;
  creating: boolean;
  createShare: (input: CreateShareInput) => Promise<{ ok: boolean; error?: string; token?: string }>;
...
export function useAnalyticsShares(userId?: string | null): UseAnalyticsSharesResult {
  ...
  const fetchShares = useCallback(async () => {
    ...
    if (ids.length > 0) {
      ...
      memberMap = ((memberRows ?? []) as MemberRow[]).reduce<Record<string, string[]>>((acc, row) => {
  ...
  const createShare = useCallback(
    async (input: CreateShareInput) => {
      ...
      if (input.visibility === "private" && cleanedEmails.length === 0) {
        setCreating(false);
        return { ok: false, error: "Private share requires at least one allowed email." };
      }
      ...
      if (shareError || !shareRow) {
        setCreating(false);
        return { ok: false, error: shareError?.message || "Failed to create share link." };
      }

      if (input.visibility === "private" && cleanedEmails.length > 0) {
        const payload = cleanedEmails.map((email) => ({ share_id: shareRow.id, email }));
        const { error: memberError } = await supabase.from("analytics_share_members").insert(payload);
        if (memberError) {
          await supabase.from("analytics_shares").delete().eq("id", shareRow.id);
          setCreating(false);
  ...
  const deleteShare = useCallback(
    async (shareId: string) => {
      const { error } = await supabase.from("analytics_shares").delete().eq("id", shareId);
